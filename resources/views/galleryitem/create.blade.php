        ...
        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
            ...
            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                    ...
                    <!--end::Item-->
                    <!--begin::Item-->
                    <li class="breadcrumb-item text-dark">Gallery Item Create</li>
                    <!--end::Item-->
                </ul>
                <!--end::Breadcrumb-->
            </div>
                ...
                @foreach (\App\Models\Language::all() as $key => $language)
                    <li class="nav-item">
                        <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3"
                            href="{{ route('galleryitem.create', [ 'lang' => $language->code]) }}">
            ...
            <div class="col-md-12">
                <div class="card card-flush">
                    <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                        <h5 class="mb-0 h6">{{ translate('Create Gallery Item') }}</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('galleryitem.store') }}" method="POST">
                                ...
                                <label for="name">{{ translate('Parent Gallery') }}</label>
                                    ...
                                    <select name="gallery_id" id="" class="form-control" required>
                                        ...
                                        @foreach ($galleries as $gallery)
                                            <option  value="{{ $gallery->id }}">{{ $gallery->getTranslation('name') }}</option>
                                        @endforeach
                                    </select>
                            </div>
                            <div class="form-group mb-3">
                                <label for="name">{{ translate('Title') }}</label>
                                <input type="text" placeholder="{{ translate('Title') }}" name="name"

            ...
            <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                ...
                <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
                        ...
                        <!--end::Item-->
                        <!--begin::Item-->
                        <li class="breadcrumb-item text-dark">Edit Room Facility</li>
                        <!--end::Item-->
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
            ...
            <div id="kt_content_container" class="container-xxl">
                <ul class="nav nav-tabs nav-fill border-light">
                    @foreach (\App\Models\Language::all() as $key => $language)
                        <li class="nav-item">
                            <a class="nav-link text-reset @if ($language->code == $lang) active @else bg-soft-dark border-light border-left-0 @endif py-3" href="{{ route('roomfacility.edit',['id'=> $roomfacility->id,'lang' => $language->code]) }}">
                    ...
                    <div class="col-md-12">
                        <div class="card card-flush">
                            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                                <h5 class="mb-0 h6">{{ translate('Edit Room Facilities') }} - {{$roomfacility->room->name}}</h5>
                            </div>
                            <div class="card-body">
                                <form action="{{ route('roomfacilities.update',$roomfacility->id) }}" method="POST">
                                    <input name="_method" type="hidden" value="PATCH">
                                    <input type="hidden" name="lang" value="{{ $lang }}">
                                    @csrf
                                    <input type="text" name="room_id" value="{{$roomfacility->room->id}}" class="form-control d-none">
                                    <div class="form-group mb-3">
                                        <label for="name">{{ translate('Name') }}</label>
                                        <input type="text" placeholder="{{ translate('Name') }}" name="name"
                                            class="form-control" autofocus value="{{$roomfacility->getTranslation("name")}}">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="name">{{ translate('Sub Name') }}</label>
                                        <input type="text" placeholder="{{ translate('Sub Name') }}" name="decription"
                                            class="form-control" value="{{$roomfacility->getTranslation("decription")}}">
                                    </div>
                                    <div class="form-group mb-3">
                                        <label for="name">{{ translate('Sorting Id') }}</label>
                                        <input type="number" placeholder="0" name="sorting_id" class="form-control" value="{{$roomfacility->sorting_id}}">
                                    </div>
                                            ...
                                            <select class="form-select mb-2 ic2t" data-hide-search="false" data-placeholder="Select an Icon" name="icon" data-live-search="true">
                                                ...
                                                @foreach (jt_get_font_icons() as $icon_key => $icon)
                                                    @php
                                                        $ico = '&#x' . $icon . ';';
                                                    @endphp
                                                    <option value="{{ $icon_key }}" {{$roomfacility->icon == $icon_key ? "selected" : ""}}>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <div class="input-group" data-toggle="aizuploader" data-type="image">
                                            ...
                                            <input type="hidden" name="image" value="{{$roomfacility->image}}" class="selected-files">
                                        </div>
